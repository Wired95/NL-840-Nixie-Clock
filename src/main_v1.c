 ...
 * Author: Nicolas
 *
 * Created on 11 juillet 2022, 18:32
 */
...
void init_IO() {
    ...
    TRISB7 = 1; //RB7 (H++) as Output PIN
    TRISB6 = 1; //RB6 (M++) as Output PIN
}
...
void main(void) {
    ...
    while (1) {
        ...
        // Time increment
        // Minutes were initially supposed to incremented each 12000 cycles (for a 5ms delay)
        // Due to the operations in the multiplexing, the clock slows with this ideal value
        // Empirical tries shows that cycles count must be xxxx +/- 5% IRL depending on quartz stability
        // Use this formula to adjust the value after running the clock for a while and measuring the time mesured by the clock and the real time
        // C = ( time_diff(clock) * C_orig ) / time_diff(real)
        // Higher quartz frequency will make the value tends to the ideal cycles count
        if (minutes_diff >= 9946) {
            minutes_diff = 0;
